...
public class MainActivity extends Activity
	...
	//GooglaApiClient 객체를 맴버필드에 담기위해
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//EditText 의 참조값을 얻어와서 맴버필드에 저장하기
		console = (EditText)findViewById(R.id.console);
		inputText = (EditText)findViewById(R.id.inputText);
		//GoogleApiClient 객체를 생성해서 맴버필드에 담기
	...
	public Collection<String> getNodes(){
		...
		NodeApi.GetConnectedNodesResult nodes=
			Wearable.NodeApi.getConnectedNodes(gClient)
				.await();
		//반복문돌면서 연결된 node 의 id 를 HashSet 에 담는다.
		for(Node node : nodes.getNodes()){
			results.add(node.getId());
		}
		//연결된 node 의 아이디 값을 담고 있는 HashSet 객체 리턴해주기
		return results;
	}
}